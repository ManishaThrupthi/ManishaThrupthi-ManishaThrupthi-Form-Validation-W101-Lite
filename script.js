const bdate = document.getElementById("dob");
bdate.addEventListener("change", () => validateDOB(bdate));
function validateDOB(bdate){
let d1=bdate.value.split("-");
let year=d1[0];
let month=d1[1];
let date=d1[2];
let birthdate = new Date(year, month, date);
let today = new Date();
let curr_Year2= today.getFullYear();
let birth_Year=birthdate.getFullYear()
let age = curr_Year2 - birth_Year;
...
if (age<18 || age>55)
{
bdate.setCustomValidity("Your age is not between 18 , 55");
...
const display=()=>{
    ...
    <th >Password</th>
    <th>Dob</th>
    <th>Checked Terms&Conditions</th>
    </tr>
    ${tableEntries}</table>`;
